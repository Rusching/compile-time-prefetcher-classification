...
const char *MapFeatureString[] = {"PC",      "Offset",      "Delta",
                                  "PC_path", "Offset_path", "Delta_path",
                                  "Address", "Page"};
...
const char *MapRewardTypeString[] = {
    "none",           "incorrect",     "correct_untimely",
    "correct_timely", "out_of_bounds", "tracker_hit"};
...
void Scooby_STEntry::update(uint64_t page, uint64_t pc, uint32_t offset,
                            uint64_t address)
{
    ...
    {
        int32_t delta = (offset > this->offsets.back())
                            ? (offset - this->offsets.back())
                            : (-1) * (this->offsets.back() - offset);
    ...
    {
        fprintf(stdout, "[TRACE] page %16llx| pc %16llx| offset %2u\n", page,
                pc, offset);
    }
}
...
uint32_t Scooby_STEntry::get_delta_sig2()
{
    ...
    for (uint32_t index = ptr; index < deltas.size(); ++index)
    {
        int sig_delta =
            (deltas[index] < 0)
                ? (((-1) * deltas[index]) + (1 << (SIG_DELTA_BIT - 1)))
                : deltas[index];
...
void Scooby_STEntry::insert_action_tracker(int32_t pref_offset)
{
    // bool found = false;
    auto it = find_if(
        action_tracker.begin(), action_tracker.end(),
        [pref_offset](ActionTracker *at) { return at->action == pref_offset; });
...
bool Scooby_STEntry::search_action_tracker(int32_t action, int32_t &conf)
{
    conf = 0;
    auto it =
        find_if(action_tracker.begin(), action_tracker.end(),
                [action](ActionTracker *at) { return at->action == action; });
    if (it != action_tracker.end())
    {
        conf = (*it)->conf;
        return true;
...
void ScoobyRecorder::record_access(uint64_t pc, uint64_t address, uint64_t page,
                                   uint32_t offset, uint8_t bw_level)
{
...
void ScoobyRecorder::record_trigger_access(uint64_t page, uint64_t pc,
                                           uint32_t offset)
{
...
void ScoobyRecorder::record_access_knowledge(Scooby_STEntry *stentry)
{
    ...
    uint64_t bmp_real_rot_value = BitmapHelper::value(
        BitmapHelper::rotate_left(bmp_real, trigger_offset));
    ...
    {
        access_bitmap_dist.insert(
            std::pair<uint64_t, uint64_t>(bmp_real_rot_value, 1));
        unique_bitmaps_seen++;
    }
    total_bitmaps_seen++;
    ...
    for (uint32_t hop = 1; hop <= MAX_HOP_COUNT; ++hop)
    {
        for (uint32_t index = 0; index < stentry->offsets.size(); ++index)
        {
            ...
            {
                delta =
                    (stentry->offsets[index + hop] - stentry->offsets[index]);
                if (delta < 0)
                    delta = (-1) * delta + 63;
...
void ScoobyRecorder::dump_stats()
{
    ...
    if (knob::scooby_access_debug)
    {
        ...
        for (auto itr = access_bitmap_dist.begin();
             itr != access_bitmap_dist.end(); ++itr)
            pairs.push_back(*itr);
        ...
        for (uint32_t index = 0; index < pairs.size(); ++index)
        {
            ...
            if (index < 20)
            {
                ...
                cout << hex << pairs[index].first << dec << ","
                     << pairs[index].second << endl;
            }
        }
        cout << "top_20_perc " << (float)top_20_occ / total_occ * 100 << "%"
             << endl;
        cout << std::endl;
...
void print_access_debug(Scooby_STEntry *stentry)
{
    ...
    {
        if (stentry->unique_pcs.find(knob::scooby_print_access_debug_pc) ==
            stentry->unique_pcs.end())
        {
            to_print = false;
        }
        else
    ...
    fprintf(stdout,
            "[ACCESS] %16llx|%16llx|%2u|%2u|%64s|%64s|%2u|%2u|%2u|%2u|%2u|",
            stentry->page, trigger_pc, trigger_offset, unique_pc_count,
            ...
            BitmapHelper::count_bits_same(stentry->bmp_real,
                                          stentry->bmp_pred), /* covered */
            BitmapHelper::count_bits_diff(stentry->bmp_real,
                                          stentry->bmp_pred), /* uncovered */
            BitmapHelper::count_bits_diff(
                stentry->bmp_pred, stentry->bmp_real) /* over prediction */
    );
    for (const uint64_t &pc : stentry->unique_pcs)
    {
        fprintf(stdout, "%llx,", pc);
    }
    fprintf(stdout, "|");
