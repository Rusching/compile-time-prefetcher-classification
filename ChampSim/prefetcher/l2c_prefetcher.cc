...
void CACHE::l2c_prefetcher_initialize()
{
	for(uint32_t index = 0; index < knob::l2c_prefetcher_types.size(); ++index)
	{
		...
		else if (!knob::l2c_prefetcher_types[index].compare("pathfinder"))
		{
			cout << "adding L2C_PREFETCHER: PATHFINDER" << endl;
			PathfinderPrefetcher *pref_pf = new PathfinderPrefetcher(knob::l2c_prefetcher_types[index]);
			prefetchers.push_back(pref_pf);
		}
		else
		{
