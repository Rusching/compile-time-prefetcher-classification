...
#define MYLOG(...)                                                             \
    LOCKED(LOGID(); fprintf(stdout, __VA_ARGS__); fprintf(stdout, "\n");)
#else
#define MYLOG(...)                                                             \
    {                                                                          \
    }
...
FeatureKnowledge::FeatureKnowledge(FeatureType feature_type, float alpha,
                                   float gamma, uint32_t actions, float weight,
                                   float weight_gradient, uint32_t num_tilings,
                                   uint32_t num_tiles, bool zero_init,
                                   uint32_t hash_type,
                                   int32_t enable_tiling_offset)
    : m_feature_type(feature_type), m_alpha(alpha), m_gamma(gamma),
      m_actions(actions), m_weight(weight), m_weight_gradient(weight_gradient),
      m_hash_type(hash_type), m_num_tilings(num_tilings),
      m_num_tiles(num_tiles),
      m_enable_tiling_offset(enable_tiling_offset ? true : false)
{
    assert(m_num_tilings <= FK_MAX_TILINGS);
    assert(m_num_tilings == 1 ||
           m_enable_tiling_offset); /* enforce the use of tiling offsets in case
                                       of multiple tilings */
...
float FeatureKnowledge::getQ(uint32_t tiling, uint32_t tile_index,
                             uint32_t action)
{
...
void FeatureKnowledge::setQ(uint32_t tiling, uint32_t tile_index,
                            uint32_t action, float value)
{
    assert(tiling < m_num_tilings);
    assert(tile_index < m_num_tiles);
    assert(action < m_actions);
...
void FeatureKnowledge::updateQ(State *state1, uint32_t action1, int32_t reward,
                               State *state2, uint32_t action2)
{
    ...
    for (uint32_t tiling = 0; tiling < m_num_tilings; ++tiling)
    {
        ...
        MYLOG("<tiling %u> Q(%s,%u) = %0.2f, R = %d, Q(%s,%u) = %0.2f, "
              "Q(%s,%u) = %0.2f",
              tiling, state1->to_string().c_str(), action1, Qsa1_old, reward,
              state2->to_string().c_str(), action2, Qsa2,
              state1->to_string().c_str(), action1, Qsa1);
    }

    float QSa1_new_overall = retrieveQ(state1, action1);
    MYLOG("<feature %s> Q(%s,%u) = %0.2f, R = %d, Q(%s,%u) = %0.2f, Q(%s,%u) = "
          "%0.2f",
          getFeatureString(m_feature_type).c_str(), state1->to_string().c_str(),
          action1, QSa1_old_overall, reward, state2->to_string().c_str(),
          action2, QSa2_old_overall, state1->to_string().c_str(), action1,
          QSa1_new_overall);

    /* tracing Q-values */
    if (knob::le_featurewise_enable_trace &&
        knob::le_featurewise_trace_feature_type == m_feature_type &&
        !knob::le_featurewise_trace_feature.compare(
            get_feature_string(state1)) &&
...
uint32_t FeatureKnowledge::get_tile_index(uint32_t tiling, State *state)
{
    ...
    case F_PC_Path_Offset_Path_Delta_Path:
        return process_PC_Path_Offset_Path_Delta_Path(tiling, pc_path,
                                                      offset_path, delta_path);
...
{
    trace_timestamp++;
    fprintf(trace, "%llu,", trace_timestamp);
    for (uint32_t action = 0; action < m_actions; ++action)
    {
