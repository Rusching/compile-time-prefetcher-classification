...
LearningEngineBasic::LearningEngineBasic(Prefetcher *parent, float alpha,
                                         ...
                                         uint32_t actions, uint32_t states,
                                         uint64_t seed, std::string policy,
                                         std::string type, bool zero_init,
                                         uint64_t early_exploration_window)
    : LearningEngineBase(parent, alpha, gamma, epsilon, actions, states, seed,
                         policy, type)
{
...
uint32_t LearningEngineBasic::chooseAction(uint32_t state)
{
    ...
    if (m_type == LearningType::SARSA && m_policy == Policy::EGreedy)
    {
            ...
            (*explore)(generator))
        {
            ...
            MYLOG("action taken %u explore, state %x, scores %s", action, state,
                  getStringQ(state).c_str());
        }
        else
        {
            ...
            MYLOG("action taken %u exploit, state %x, scores %s", action, state,
                  getStringQ(state).c_str());
        }
    }
    else
    ...
    if (knob::le_enable_action_trace &&
        action_trace_interval++ == knob::le_action_trace_interval)
    {
...
void LearningEngineBasic::learn(uint32_t state1, uint32_t action1,
                                int32_t reward, uint32_t state2,
                                uint32_t action2)
{
    stats.learn.called++;
    if (m_type == LearningType::SARSA && m_policy == Policy::EGreedy)
    {
        ...
        MYLOG("Q(%x,%u) = %.2f, R = %d, Q(%x,%u) = %.2f, Q(%x,%u) = %.2f",
              state1, action1, Qsa1_old, reward, state2, action2, Qsa2, state1,
              action1, Qsa1);
        MYLOG("state %x, scores %s", state1, getStringQ(state1).c_str());

        if (knob::le_enable_trace && state1 == knob::le_trace_state &&
            trace_interval++ == knob::le_trace_interval)
        {
...
void LearningEngineBasic::dump_stats()
{
    ...
    for (uint32_t action = 0; action < m_actions; ++action)
    {
        fprintf(stdout, "learning_engine.action.index_%d_explored %llu\n",
                scooby->getAction(action), stats.action.dist[action][0]);
        fprintf(stdout, "learning_engine.action.index_%d_exploited %llu\n",
...
void LearningEngineBasic::plot_scores()
{
    ...
    for (uint32_t index = 0; index < knob::le_plot_actions.size(); ++index)
    {
        if (index)
            fprintf(script, ", ");
        fprintf(script, "'%s' using 1:%u with lines title \"action_index[%u]\"",
                knob::le_trace_file_name.c_str(),
                (knob::le_plot_actions[index] + 2),
                knob::le_plot_actions[index]);
    }
    fprintf(script, "\n");
    fclose(script);
